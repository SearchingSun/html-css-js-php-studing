...
<!--Посмотрите функцию header("Location: адрес");                                              WTF &!&!&!&&!&!&!&!&&!&!-->
...
function auth($login, $pwd, $data) {
    ...
    foreach ($data as &$value){
        if ($login == $value[0] && password_verify($pwd, $value[1])) {
//        $_SESSION['auth'] = true;
...
function revriteCSV($arr){
    $handle2 = fopen("polzovateli.csv", "w+");
    foreach ($arr as $value){
        fputcsv($handle2, $value, ",",'"');
    }
    fclose($handle2);
}

$hash = password_hash($post['pwd'], PASSWORD_DEFAULT );
var_dump($hash);
$qwe = 'ffsdf';
var_dump(password_verify($qwe, $hash));



var_dump('fsddfgdfggdfgdfgdfdgfdgfdgfgfd');
...
<?php if (isset($_SESSION['login'])): ?>
    ...
    <h2><a href="second.php">др.страница</a></h2>
<?php else: ?>
    ...
    <form method="post" action="hw4_2.php">
        ...
        <h2><a href="registration.php">Зарегистрироваться</a></h2>
    </form>
<?php endif; ?>